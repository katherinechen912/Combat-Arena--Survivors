...
# License authorization state (set by plugin.gd)
...
func execute(method: String, params: Dictionary) -> Dictionary:
	...
	if not is_authorized:
		return {
			"error": {
				"code": -32001,
				"message": "MCP Pro is not activated. Please add your license key to addons/godot_mcp/.license_key and restart Godot."
			}
		}
